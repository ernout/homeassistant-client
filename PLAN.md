...
- ✅ **Fase 3 afgerond**: live updates via `subscribe_entities` (compact diff-formaat, gefilterd op de dashboard-entities, reconnect met backoff, stopt bij schermverlaten/app-pauze) + instellingenscherm met per-server toggles voor locatie (standaard uit) en batterij.
- 🔎 **Push-onderzoek (fase 4) — LightOS gebruikt UnifiedPush, maar de notificatie-UI ontbreekt nog**:
  - Werkt: een externe server kan **rechtstreeks** naar het UnifiedPush-endpoint POSTen; de payload komt byte-voor-byte binnen in `onPushNotification`. Encryptie is optioneel (de connector valt terug op plaintext) en is momenteel sowieso onmogelijk: `LightPushManager` bewaart alleen de endpoint-URL, niet de `pubKeySet` (`// TODO save pubKeySet`). VAPID staat uit (geen `vapidPublicKey` in `local.properties`). Geen relay nodig voor bezorging.
  - **Blokkade**: Light-medewerker `dupontgu` in [discussion #111](https://github.com/orgs/lightphone/discussions/111): *"We haven't published a version of LightOS that actually shows notifs from external tools. In progress!"* De tool kan dus wél gewekt worden, maar LightOS toont de melding nog niet aan de gebruiker. Ook [issue #128](https://github.com/lightphone/light-sdk/issues/128): notificatie-indicatoren zijn in de maak, "sticky" notificaties niet.
  - Aandachtspunten voor later: HA's `mobile_app` doet `await response.json()` vóór de statuscheck, dus het endpoint moet JSON teruggeven, anders logt HA een fout bij elke melding (bezorging lukt wel). Light's productie-endpoint is closed source: responsetype, headers en rate limits onbekend. `supports_push` vereist zowel `push_url` als een (dummy) `push_token` in `app_data`. UnifiedPush-limiet is 4096 bytes. En: `ToolEntryPoint` moet `enablePushNotifications = true` overriden, anders wordt de tool niet voor remote push geregistreerd.
  - Alternatief dat nu al kan: `mobile_app/push_notification_channel` over de WebSocket (werkt zolang de app open is, geen limieten). Let op: als dat kanaal verbonden is, kiest HA dat en POST het niet naar `push_url`.
- Volgende: fase 4-camera (`camera_proxy`-snapshots — kan direct), push zodra Light de notificatie-UI uitrolt; fase 5 (multi-server UI, polish). Periodieke locatie via `LightWork` staat ook nog open.
